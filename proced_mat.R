...
depart_asp <- procedencia.R[,3]
ciudad_asp <- procedencia.R[,4]
codept_asp<-procedencia.R[,8]
codept_asp <-as.integer(as.character(codept_asp))

codecity_asp <-procedencia.R[,10]
codecity_asp <-as.integer(as.character(codecity_asp))
long_asp <- procedencia.R[,11]
lat_asp <- procedencia.R[,12]
indi <- procedencia.R[,13]<-1
